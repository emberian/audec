...
# Opening an editor makes its pane the active one and Close is offered exactly
# where the workspace accepts it, so closing the pane just opened is one verb.
# The reply's `notice` is the refusal when there is one.
close_active_pane() {
  local reply=$(act audec.workspace.close)
  echo "   $reply"
  echo "$reply" | grep -q '"notice": null'
}
echo "0. baseline"; pv
...
echo "3. close the sampler pane -> accepted, and nothing is playing, no pad held"
close_active_pane && echo "   closed" || echo "   Close was refused"
sleep 2; pv
echo "4. open and close twice more, to show nothing accumulates"
for i in 1 2; do act audec.editor.sampler >/dev/null; sleep 2; close_active_pane >/dev/null; sleep 1; done; pv
