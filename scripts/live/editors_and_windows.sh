...
'; }
# The overview is view 1 and is the document`s only pinned pane.
walk_to_overview() {
  for i in {1..14}; do
    [[ "$(av)" == "1" ]] && return 0
    if (( i % 5 == 0 )); then act audec.workspace.next_pane >/dev/null; else act audec.workspace.next_tab >/dev/null; fi
    sleep 1
  done
  [[ "$(av)" == "1" ]]
}
...
[[ "$after" != "None" ]] && echo "   OK: a pane is active" || echo "   FAIL: active_view is still None"
echo "   Close is offered for that pane:"; close_row
echo "7. walk to the overview: the projection refuses Close there, with its reason"
walk_to_overview && echo "   active_view is now $(av) (the overview)" || echo "   FAIL: could not reach the overview"
close_row
echo "   invoking it anyway reports the same refusal:"; act audec.workspace.close
echo "8. next pane"
before=$(av); act audec.workspace.next_pane >/dev/null; sleep 1; after=$(av)
echo "   audec.workspace.next_pane: active_view $before -> $after"
echo "9. next tab, then previous tab"
before=$after; act audec.workspace.next_tab >/dev/null; sleep 1; after=$(av)
echo "   audec.workspace.next_tab: active_view $before -> $after"
[[ "$before" != "$after" ]] && echo "   OK: active_view followed the focused pane" || echo "   FAIL: active_view did not move"
before=$after; act audec.workspace.previous_tab >/dev/null; sleep 1; after=$(av)
echo "   audec.workspace.previous_tab: active_view $before -> $after"
