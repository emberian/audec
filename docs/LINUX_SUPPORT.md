...
| Audio output | **build-gated only** | Rodio/CPAL select ALSA on Linux; `libasound2-dev` is installed for builds. No hosted runner supplies a real ALSA/PipeWire device, so device opening, latency, XRUN behavior, and routing are unverified. |
