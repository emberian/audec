...
| Track rename / delete / reorder / lock; per-track fader | `arrangement.rs:691 PutTrack` | arrangement track header | `ArrangementAction` has only `CreateTrack`; header M/S are badges, not buttons | M |
...
## Catalog entries that reach nothing
...
Classes: (a) acknowledges but changes nothing; (b) pane-local where the
