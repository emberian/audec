...
| `coverage` presenter + tile cache **coverage artifacts published per retained comparison 2026-09-06; presenter/tile cache still unwired** | `CoverageWorkbenchPresenter`, `CoverageTileCache`, `clear_comparison` | over-gained construction lights excess instead of hiding it; span analysis partitions the field exactly | explained energy reaches the UI; the Components lens shows an unrelated NMF "% explained" next to it | cached coverage overlay while scrubbing | S | **PARK**, but reconcile the two "% explained" numbers now |
...
| `reconstruction` + `reconstruction_apply` **apply_automation rides clip gain, refuses the rest by name (2026-09-06)** | ranked deterministic proposals with anonymous families and a mandatory residual layer; atomic apply | families never become instrument labels; every proposal keeps a residual layer | producer never runs; `artifact_promotion_bridge` promotes one candidate | "three readings of this loop, each with its leftover audio kept" | L (146 items) | apply half **SUPERSEDED**; ranking half **PARK**; keep the id vocabulary |
| `reading_query_view` presentation **residual guide + readings import/export landed 2026-09-06 (cycle 3)** | `ReadingQueryPresentation`, `install_residual_guide`, `accept_refusal` | notice never conflates a planned command with success | the reaction is wired, nothing installs; refusals arrive as strings | refusal reasons a musician can act on | S | **PARK**; `accept_refusal` is a small WIRE |
