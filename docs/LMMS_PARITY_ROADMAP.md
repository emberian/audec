...
Loaded material now creates a validated `LiveProject`. Its arrangement, mixer,
automation, media pool, decoded PCM, revisions, and transport render share one
aggregate state; Play recompiles dirty edits in the background. The sequencer
attaches to real project patterns when present, but empty-project pattern
creation and reconstruction-promotion UI are still missing. Save/open and
export have lossless/atomic cores but no user dialogs yet.
...
| --- | --- | --- | --- |
| Open/import a recording | Symphonia provides one bounded WAV/FLAC/MP3/Vorbis decode seam; the UI-neutral import path materializes canonical project-rate PCM through a persisted, fingerprint-verified Rubato recipe while retaining encoded-source identity for relink. | **Core implemented; picker still FLAC-first** | Route every picker/drop entry through the materialized asset transaction, add AIFF fixtures, and expose import/relink diagnostics in the docked Library. |
| Project save/open | Versioned envelope, lossless constructive-domain codecs, asset route intent, atomic writes, autosave and recovery exist. | **Core only** | Save/Save As/open/recent/recovery UI; complete AIR payload codec and workspace restoration in the application. |
| Asset browser and relinking | Source-backed searchable media pool shows provenance, decoded facts, route, content identity and usage; duplicate/missing/relink core exists. | **Implemented for inspection/audition** | Dock it with all editors; drag/drop import, waveform preview, tags/collections, and missing-media repair actions. |
| Global transport | Exact frame play/pause/seek/loop, independent preview, and background recompilation of dirty `LiveProject` edits are active. | **Implemented audition loop** | Streaming graph transport, tempo display, metronome, count-in, punch, record-arm, preroll, and editor-safe focus rules. |
| Multitrack arrangement | Persistent project-owned tracks/clips and non-destructive transactions; GPUI editor has selection, mouse/keyboard edit operations, zoom/pan/fit/snap and undo. | **Implemented for loaded source** | Track create/reorder/color/height, imports, marquee/tool modes, drag trim/slip/fade/crossfade, markers, loop/punch, minimap, and clip waveforms. |
| Audio clips | Source ranges, gain, reverse, channel mapping, loop modes, fades and rational stretch metadata; loaded-source edits recompile through the asset-backed live graph. | **Implemented for loaded source** | Waveform proxies, drag/drop imports, clip gain/fades/crossfades UI, channel/warp controls, offline high-quality stretch/pitch algorithms. |
...
| Piano roll | Notes, velocity, articulation and per-note expression core; GPUI editor attaches to real project patterns and supports add/select/move/resize/delete, quantize, grid, zoom/pan and undo. | **Project-bound core/UI** | Empty-project pattern creation/selection; note audition; velocity/expression lanes, scale/chord tools, lasso, copy/paste, record-to-pattern, inspector. |
| Beat/step/drum sequencer | Step lanes/events include triggers, probability, ratchets, microtiming/gate/pitch/gain/pan/swing; GPUI editor attaches to real project step patterns. | **Project-bound core/UI** | Empty-project pattern/lane creation, pad/sample mapping and audition, pattern clips, recording, per-step automation, browser drag-to-pad. |
...
| Mixer and routing | Buses, sends, inserts, gain/pan/mute/solo, cycle checks, latency plan and history exist. The GPUI surface edits the live project and changes are heard after background recompilation. | **Project-bound prototype** | Real post-DSP meters, routing/send editor, groups/returns, record/monitor state, continuous graph publication and plugin execution. |
...
| Automation/modulation | Typed addresses, sample/beat-domain curves, interpolation, smoothing, compile-without-allocation and write-mode core; GPUI curve editor edits the live project graph. | **Project-bound prototype** | Create/bind lanes from controls, project/clip lane selection, record/touch/latch/write, and continuous live DSP application. |
...
| Realtime audio engine | Source playback and preview bus are live; dirty aggregate edits compile and render asynchronously into a replacement sample-accurate transport. | **Audition prototype** | Incremental immutable graph publication to callback, streaming assets/instruments/plugins/automation/mixer, lock/allocation-free callback, meters/xrun diagnostics, device switching. |
