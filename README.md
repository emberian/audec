...
> **Project status:** audec is an early macOS alpha under active development. A loaded recording now inhabits a validated aggregate project with dynamic docked/floating views, project-level undo and journal checkpoints, lossless save/recovery, exact ranged sampling, pads and patterns, deterministic sampler playback, and coherent render publication. The live Project/Library/Investigate/Readings Explorer and six-section Inspector address the same typed objects used by production and reverse-analysis panes; creating a sample, sliced kit, beat, pattern placement, automation occurrence, or Loom construction reveals the exact result instead of ending at a revision badge. Timeline selection, loop, playhead, follow mode, and local viewport are separate state, while Browser and pad audition use authoritative material through an owner-scoped preview bus. Content-addressed render tiles now persist in a verified OS-native cache across relaunches; deeper sampler/pattern/arrangement workflows, reverse working documents, multiwindow session layout, and editable deprojection source programs have tested foundations. Imported analysis now remains part of the aggregate AIR with exact asset/clip bindings, and revision-pinned Reading Explorer results can reveal typed AIR/project identities or audition source, construction, and residual through the shared renderer and transport. Large Components, Rhythm, HPSS, and Loom recipes are now content-addressed away from the GPUI thread before their short generation-checked scheduler admission. Its primary library corpus currently has 1,290 passing checks, zero failures, and two explicitly ignored cases, with the feature-gated direct-CPAL preview regressions plus fake-worker and native plug-in process suites also covered; real desktop musician validation remains incomplete. CLAP execution is an isolated supervised process with a controlled native fixture, not a general third-party compatibility claim; downloaded ML models remain incomplete. It is not yet a replacement for an established DAW or source-separation product.
...
- **Rhythm deprojection** — asynchronous multiband novelty analysis, ranked tempo alternatives, beat/downbeat and pattern-start evidence, anonymous hit families, exact source spans, and auditionable family medoids. Up to four pulse candidates can be explicitly adopted as the project tempo; the chosen integer tempo map immediately governs arrangement rulers, pattern placement, render, persistence, and undo while later authored tempo changes remain intact. Families remain unnamed until stronger evidence exists.
...
- **Loom** — aligned, phase-preserving excerpts from recurring attacks deprojected into an editable event sequence. Clusters and events can be muted, shifted, or gain-adjusted; audec immediately overlap-adds a new render and exposes the residual. **Make Pattern** turns the current reading into ordinary generated media, anonymous sampler pads, an editable step pattern, a placed arrangement clip, and mixer routing in one undoable project command.
...
Reading/query result rows obey that same authority. **Audition** resolves one frozen project publication into either an exact source citation or a retained comparison channel, then locates the shared transport at the row's focus. **Reveal** publishes the row's durable AIR or project identity plus any project-local time/frequency geometry into semantic selection. Ambiguous explanation mappings, stale revisions, and unsupported band-isolated source audition are refused explicitly rather than guessed.

The global transport displays the authoritative project base tempo and meter. Its `−` / `+` BPM controls, the Transport menu, and command palette all lower through the same typed action into one persistent, undoable tempo-map edit. Sequencer publications refresh every arrangement ruler and snap grid, and tempo changes alter the exact audition-render identity rather than only repainting chrome.
...
The Loom construction preserves that distinction after promotion. Every generated zone points back to the exact artifact-scoped evidence and analyzed source extent; hand-edited events mark the resulting pattern as diverged rather than rewriting the inference. Repeating the same promotion deduplicates bit-identical template media. Undo removes the kit, pattern, arrangement placement, routing, and runtime PCM cohort together, and redo restores the same audible construction. Save publishes those templates as immutable canonical PCM inside the `.audec` package; reopen verifies their content identity, restores their exact floating-point samples, and renders the same sampler-backed construction.
...
- **Piano roll and drum sequencer** — targetable note and step-pattern views backed by the PPQ sequencer, including pattern lifecycle, trigger bindings, quantize, swing, microtiming-aware scheduling, deterministic performance seeds, and a compact pattern-expression language. **Place @ Cursor** publishes the current pattern onto the arrangement at the nearest sixteenth under the project tempo map, creates any required track and mixer routing atomically, and retargets the editor to the exact durable occurrence.
...
| **Place @ Cursor** (pattern editor) | Place the current pattern at the global transport cursor and reveal its arrangement occurrence |
...
| `L` | Toggle the loop |
| `−` / `+` beside BPM | Decrease or increase the project base tempo by one BPM |
| **Sample** / `S` | Turn the timeline's labeled material range into an exact ranged sample and reveal its Instrument/Pad |
| **Slice** / `⇧S` | Create and reveal an eight-pad Instrument from that range |
| **Beat** / `B` | Create samples, pads, an editable step pattern and placed occurrence, then reveal the requested destination |

`Sample`, `Slice`, and `Beat` also appear directly beside the active range on
the overview timeline. A new non-empty selection takes material precedence over
a different older loop; the loop remains visible and audible until explicitly
replaced. When selection and loop have identical bounds, the range retains its
loop provenance.
...
Manual pan or zoom disengages follow mode. In Waterfall, `FFT−` / `FFT+` and **Win** recompute the spectrum; `F−` / `F+` change the frequency viewport; and `D−` / `D+` and `R−` / `R+` change the dB ceiling and range from retained numeric values. In Decomposition, **Analyze view** reruns HPSS for the visible span. In Loom, the audition controls compare the source mix, reconstruction, residual, and selected template; **Make Pattern** publishes the edited reading into the ordinary Sampler, Pattern, Arrange, and Mixer domains and opens the resulting sequencer target.
...
FLAC source
  └── retained PCM + waveform pyramid
      ...
      └── recurrence events → Loom templates → render + residual
                                └── Apply → pads + pattern + arrangement + mixer
...
| `src/audio.rs` / `src/audio_host.rs` | Sample-exact transport, independent project/audition buses, and a backend-neutral host-control contract shared with compiled-graph devices |
...
| `src/reading_effect_bridge.rs` | Revision-pinned reading/query resolution into shared source/comparison audition and semantic reveal |
