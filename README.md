...
> **Project status:** audec is an early macOS alpha under active development. The current, verified state of the tree (what works on the desktop, what was found by driving the live app, known holes, and the working program) is kept in [docs/STATE.md](docs/STATE.md); the paragraphs below describe the intended shape of the product and are not all verified on a real desktop session. The library corpus has about 1,370 passing checks. Real-material desktop validation now runs through the opt-in control socket described in STATE.md. CLAP execution is an isolated supervised process with a controlled native fixture, not a general third-party compatibility claim; downloaded ML models remain incomplete. It is not yet a replacement for an established DAW or source-separation product.
...
- **Waterfall** — a retained numeric dB field with independent time and log-frequency viewports, adjustable FFT size, window function, dB ceiling, and display range, and an FFT/constant-Q toggle (24 bins per octave) so low notes resolve instead of smearing. Analysis settings recompute the field instead of merely stretching an image; the spectrum choices persist across relaunches.
...
- **Components** — deterministic convolutional NMF: each component is a recurring gesture (a frequency-by-lag template over eight frames) with its activation history, presented as a mixed-audio hypothesis with fit information rather than an instrument name.
